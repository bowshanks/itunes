angular.module('itunes').controller('mainCtrl', function($scope, itunesService){
  ...
  $scope.gridOptions = {
      data: 'songData',
      height: '110px',
      sortInfo: {fields: ['Artist', 'Song', 'Collection', 'Type', 'Genre'], directions: ['asc']},
      columnDefs: [
        {field: 'Play', displayName: 'Play', width: '40px', cellTemplate: '<div class="ngCellText" ng-class="col.colIndex()"><a href="{{row.getProperty(col.field)}}"><img src="http://www.icty.org/x/image/Miscellaneous/play_icon30x30.png"></a></div>'},
        {field: 'Artist', displayName: 'Artist'},
        {field: 'Song', displayName: 'Song'},
        {field: 'trackLength', displayName: 'Duration'},
  ...
  //First inject itunesService into your controller.
    ...
    //code here
  $scope.getArtistInfo = function(term, mediatype, limit, explicit){
    itunesService.getArtistInfo(term, mediatype, limit, explicit).then(function(response){
  ...
  $scope.explicit = 'Yes';
    //Code here
    $scope.getSongData = function(term, mediatype, limit, explicit){

      $scope.getArtistInfo(term, mediatype, limit, explicit);

      $scope.gridOptions.ngGrid.config.sortInfo = {fields: ['Artist', 'Song', 'Collection', 'Type', 'Genre'], directions: ['asc']};
    }
